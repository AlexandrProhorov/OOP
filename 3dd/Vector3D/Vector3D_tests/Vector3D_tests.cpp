...
TEST_CASE("Operations combined with assignment")
{
	...
	SECTION("Division")
	{
		std::domain_error VECTOR_PROCESSING_ERROR("Failed to process the vector");
		thirdVector /= 2;
		...
		REQUIRE_THROWS(secondVector /= 0, VECTOR_PROCESSING_ERROR);
	}
}
...
TEST_CASE("Check vector functions")
{
	...
	SECTION("Check vector normalization")
	{
		...
		std::domain_error NORMALIZE_VECTOR_ERR("Failed");
		Vector3D vectors{};
		REQUIRE_THROWS(Normalize(vectors), NORMALIZE_VECTOR_ERR);
	}
